...
class BxDolJsonUtil {
    ...
    function fillProfileArray($a, $sImage = 'icon', $iIdViewer = 0) {
        ...
        $aRet = array(
            ...
            'Online' => new xmlrpcval(((time() - strtotime($a['DateLastNav'])) > 300) ? 0 : 1),
    ...
    function checkLogin($sUser, $sPwd) {
        $iId = (int) BxDolJsonUtil::getIdByNickname($sUser);
    ...
    function setMobileStatus($iId) {        
        mysql_query("UPDATE `Profiles` SET `Status` = 'Active' WHERE `ID` = '$iId'");
    }

    function getLoginData($iId) {
        $aRes = db_arr("SELECT `NickName`,`Password` FROM `Profiles` WHERE `ID` = '$iId' LIMIT 1");
        return $aRes;
    }
