...
class BxGroupsModule extends BxDolTwigModule
{
    ...
    // Api custom
    function ApiGetNews() {
        $aNews = $this->_oDb->GetAllNews();
        foreach ($aNews as $item) {
            $aResult[] = $this->_oTemplate->unit($item, '');
        }
        return $aResult;
    }

    function ApiGetGroups() {
        $aNews = $this->_oDb->GetAllNews(); // the same table
//        foreach ($aNews as $item) {
//            $aResult[] = $this->_oTemplate->unit($item, '');
//        }
//        return $aResult;
    ...
    function getGroupById($iuId) {        
        return $this->_oDb->getGroupById($iuId);
    }    

    function ApiAddGroup($sTitle, $sDesc, $sUri, $iUser) {                
        return $this->_oDb->ApiAddGroup($sTitle, $sDesc, $sUri, $iUser);
    }
}
