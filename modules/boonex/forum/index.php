...
switch ($action) {
    ...
    case 'edit_category':
        $orca_admin = new Admin ();
        transCheck ($orca_admin->editCategory ((int)$_GET['cat_id']), $gConf['dir']['xsl'] . 'edit_cat_form.xsl', $_GET['trans']);
        break;

    case 'edit_category_submit':
        $orca_admin = new Admin ();
        transCheck ($orca_admin->editCategorySubmit ((int)$_GET['cat_id'], $_GET['cat_name'], (int)$_GET['cat_order'], (int)$_GET['cat_expanded']), '', 0);
        break;

    case 'edit_forum':
        $orca_admin = new Admin ();
        transCheck ($orca_admin->editForum ((int)$_GET['forum_id'], $_GET['cat_id']), $gConf['dir']['xsl'] . 'edit_forum_form.xsl', $_GET['trans']);
        break;

    case 'edit_forum_submit':
        $orca_admin = new Admin ();
        transCheck ($orca_admin->editFormSubmit ((int)$_GET['cat_id'], (int)$_GET['forum_id'], $_GET['title'], $_GET['desc'], $_GET['type'], (int)$_GET['order']), '', 0);
        break;
...
*/
    ...
    case 'show_hidden_topics':
        transCheck ($f->getHiddenTopicsXML(false, (int)$_GET['start']), $gConf['dir']['xsl'] . 'hidden_topics.xsl', $_GET['trans']);
        break;

    case 'show_my_threads':
        transCheck ($f->getMyThreadsXML(false, (int)$_GET['start']), $gConf['dir']['xsl'] . 'my_topics.xsl', $_GET['trans']);
        break;

    case 'show_my_flags':
        transCheck ($f->getMyFlagsXML(false, (int)$_GET['start']), $gConf['dir']['xsl'] . 'flagged_topics.xsl', $_GET['trans']);
        break;
    ...
    case 'edit_post_xml':
        transcheck ($f->editPostXml ((int)$_GET['post_id'], $_GET['topic_id']), $gConf['dir']['xsl'] . 'edit_post.xsl', $_GET['trans']);
        break;
    ...
    case 'move_topic_form':
        transCheck ($f->moveTopicForm ((int)$_GET['topic_id']), $gConf['dir']['xsl'] . 'move_topic_form.xsl', $_GET['trans']);
        break;

    case 'move_topic_submit':
        transCheck ($f->moveTopicSubmit((int)$_GET['topic_id'], (int)$_GET['forum_id'], (int)$_GET['old_forum_id'], (int)$_GET['goto_new_location']), '', 0);
        break;
    ...
    case 'search':
        transCheck ($f->getSearchResultsXML($_GET['text'], $_GET['type'], (int)$_GET['forum'], $_GET['u'], $_GET['disp'], $_GET['start']), $gConf['dir']['xsl'] . 'search.xsl', $_GET['trans']);
        break;
    ...
    case 'recent_topics':
        transCheck ($f->getRecentTopicsXML(false, (int)$_GET['start']), $gConf['dir']['xsl'] . 'recent_topics.xsl', $_GET['trans']);
        break;
    ...
    case 'goto':
        switch (true) {
            ...
            case (isset($_GET['search'])):
                transCheck ($f->getSearchXML(true), $gConf['dir']['xsl'] . 'search_form_main.xsl', $_GET['debug'] ? 0 : 1);
                break;
