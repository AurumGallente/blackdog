...
class BxEventsDb extends BxDolTwigModuleDb {
    ...
    function getAllEvents($iGroupId) {
        return $this->getAll("SELECT * FROM `" . $this->_sPrefix . "main` WHERE `group_id`=".$iGroupId);
    }

    function getEventsById($sId) {                        
        $sql = 'SELECT * FROM `' . $this->_sPrefix . 'main` WHERE `beacon` = "' . $sId . '" AND `EventStart` <= UNIX_TIMESTAMP(NOW()) AND `EventEnd` >= UNIX_TIMESTAMP(NOW())';
        return $this->getAll($sql);
    }

    function ApiAddEvent($sTitle, $sDesc, $sPlace, $iGroup, $iUser){
        $sqlQuery = "INSERT INTO bx_events_main (Title, Description, Status, Place, EventStart, EventEnd, ResponsibleID, Tags, Categories, group_id) "
                . "VALUES ('$sTitle' ,'$sDesc', 'approved' ,'$sPlace' ,UNIX_TIMESTAMP(NOW()) ,UNIX_TIMESTAMP(NOW()) , $iUser,' ',' ',$iGroup )";                
