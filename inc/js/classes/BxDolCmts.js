...
function BxDolCmts (options) {
    ...
    $('#cmts-box-' + this._sSystem + '-' + this._iObjId).click(function(event) {
    	...
    	if (iRate != 0 && !$(event.target).parents('.cmt-rate').hasClass('cmt-rate-disabled')) {    		            		
			var e = $(event.target).closest('.cmt-rate').find('.cmt-points').get();
			$this._rateComment(e, parseInt(oLink.attr('id').substr(8)), iRate);
                        
                        var iStart = 0;
                        var iPerPage = parseInt($('.cmt-pages select[name="per_page"]').val());
                        if($('.cmt-show-more .active_page').length > 0)
                            iStart = (parseInt($('.cmt-show-more .active_page').text()) - 1) * iPerPage;
                        $this.changePage(iStart, iPerPage);
    	}
        if(iRate != 0 &&  $(event.target).parents('.cmt-rate').hasClass('cmt-rate-disabled')) {
		    ...
		    jQuery.post (
		        ...
		        function (s) {
        			...
        			if(jQuery.trim(s).length)
        				...
        				$(e).closest('.cmt-rate').addClass('cmt-rate-disabled');
        			}
        			else if(iRate == -1) {
