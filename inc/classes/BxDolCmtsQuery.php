...
class BxDolCmtsQuery extends BxDolDb
{
    ...
    function getComments ($iId, $iCmtParentId = 0, $iAuthorId = 0, $sCmtOrder = 'ASC', $iStart = 0, $iCount = -1)
    {
        ...
        $a = $this->getAll("SELECT
            ...
            ORDER BY " . (strtoupper($sCmtOrder) == 'TREND' ? '(LOG10(IF (ABS(c.cmt_rate) <= 1, 1, ABS(c.cmt_rate))) - ABS(TIMESTAMPDIFF(SECOND,  c.cmt_time, NOW() ))/45000 + SIGN(c.cmt_rate)) DESC, ' : '') . " `c`.`cmt_time` " . (strtoupper($sCmtOrder) == 'ASC' ? 'ASC' : 'DESC') . ($iCount != -1 ? ' LIMIT ' . $iStart . ', ' . $iCount : ''));
