...
function getMatchFields()
{
    ...
    return $oDb->fromCache('sys_profile_match_fields', 'getAllWithKey',
        'SELECT `ID`, `Name`, `MatchField`, `MatchPercent`, `Type` FROM `sys_profile_fields` WHERE `MatchPercent` > 0', 'ID');
}
    ...
    foreach ($aMathFields as $sKey => $aFields) {
        ...
        if ($aProf[$aFields['Name']] && $aMathFields[$aFields['MatchField']]['Name']) {
            if ($aMathFields[$aFields['MatchField']]['Name'] == 'DateOfBirth') {
                ...
                $sCond = "FIND_IN_SET('" . process_db_input($aProf[$aFields['Name']], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION) . "', `{$aMathFields[$aFields['MatchField']]['Name']}`)";
            } elseif ($aMathFields[$aFields['MatchField']]['Type'] != 'select_set' && $aFields['Type'] == 'select_set') {
                $sCond = "FIND_IN_SET(`{$aMathFields[$aFields['MatchField']]['Name']}`, '" . process_db_input($aProf[$aFields['Name']], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION) . "')";
            } elseif ($aMathFields[$aFields['MatchField']]['Type'] == 'select_set' && $aFields['Type'] == 'select_set') {
                $a = explode(',', $aProf[$aFields['Name']]);
                if ($a) {
                    $sCond = '(';
                    foreach ($a as $sVal) {
                        $sCond .= "FIND_IN_SET('" . process_db_input($sVal, BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION) . "', `{$aMathFields[$aFields['MatchField']]['Name']}`) OR ";
                    }
